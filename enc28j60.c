 ...
 *  adapted for use with A500-sd-plus controller by Dennis van Weeren (2025)
 *
...
#define ENC28J60_LED_MODE					0x35d2		/*!< LEDA = duplex status, LEDB = Link status + TX/RX activity*/
...
static int enc28j60_set_bank(spi_t *spi, uint8_t addr);
static int enc28j60_set_bits(spi_t *spi, uint8_t addr, uint8_t val);
static int enc28j60_clear_bits(spi_t *spi, uint8_t addr, uint8_t val);
static int enc28j60_write_reg(spi_t *spi, uint8_t addr, uint8_t val);
static int enc28j60_read_reg(spi_t *spi, uint8_t addr);
static int enc28j60_write_reg16(spi_t *spi, uint8_t addr, uint16_t val);
static int enc28j60_read_reg16(spi_t *spi, uint8_t addr);
static int enc28j60_write_phy(spi_t *spi, uint8_t addr, uint16_t val);
static int enc28j60_read_phy(spi_t *spi, uint8_t addr);
static int enc28j60_write_buf(spi_t *spi, const uint8_t *buf, unsigned int length);
static int enc28j60_read_buf(spi_t *spi, uint8_t *buf, unsigned int length);
...
static int enc28j60_read_reg(spi_t *spi, uint8_t addr)
{
	...
	spi_read(spi, rxbuf, (addr & ENC28J60_MACREG) ? 2 : 1);
	spi_deselect();
...
static int enc28j60_write_reg16(spi_t *spi, uint8_t addr, uint16_t val)
{
	...
	status |= enc28j60_write_reg(spi, addr + 0, val & 0xff);
	status |= enc28j60_write_reg(spi, addr + 1, val >> 8);
	return status;
}
...
static int enc28j60_read_reg16(spi_t *spi, uint8_t addr)
{
	...
	l = enc28j60_read_reg(spi, addr + 0);
	h = enc28j60_read_reg(spi, addr + 1);
	if (l < 0 || h < 0) {
		return -1;
	}
	return (l | (h << 8));
...
static int enc28j60_write_phy(spi_t *spi, uint8_t addr, uint16_t val)
{
	...
	/// \todo Check for busy
	/// \bug Timeout?
	while (enc28j60_read_reg(spi, MISTAT) & MISTAT_BUSY) {}

	status |= enc28j60_write_reg(spi, MIREGADR, addr & 0x1f);
	status |= enc28j60_write_reg16(spi, MIWRL, val);
	return status;
}
...
static int enc28j60_read_phy(spi_t *spi, uint8_t addr)
{
	...
	status |= enc28j60_write_reg(spi, MIREGADR, addr & 0x1f);
	status |= enc28j60_write_reg(spi, MICMD, MICMD_MIIRD);
	...
	// Stop reading
	status |= enc28j60_write_reg(spi, MICMD, 0);
	if (status < 0) {
		return -1;
	}

	// Read the register contents
	return enc28j60_read_reg16(spi, MIRDL);
}
...
{
	spi_select(spi);
	spi_write(spi, (const uint8_t[]){ ENC28J60_SPI_WBM }, 1);
	spi_write(spi, buf, length);
	spi_deselect();
	return 0;
}
...
{
	spi_select(spi);
	spi_write(spi, (const uint8_t[]){ ENC28J60_SPI_RBM }, 1);
	spi_read(spi, buf, length);
	spi_deselect();
	return 0;
}
...
int nic_init(spi_t *spi)
{
	...
	spi_select(spi);
	spi_write(spi, (const uint8_t[]){ ENC28J60_SPI_SRC }, 1);
	spi_deselect();
	timer_wait(TIMER_MILLIS(10));
	...
	enc28j60_write_reg16(spi, ERXSTL, RXSTART_INIT);
	enc28j60_write_reg16(spi, ERXNDL, RXSTOP_INIT);
	enc28j60_write_reg16(spi, ERXRDPTL, RXSTART_INIT);
	enc28j60_write_reg16(spi, ETXSTL, TXSTART_INIT);

	// Bank 2

	// Initialise MAC for half-duplex operation
	enc28j60_write_reg(spi, MACON1, MACON1_MARXEN | MACON1_TXPAUS | MACON1_RXPAUS);
	// Some code seems to write 0 to this undocumented register
	enc28j60_write_reg(spi, MACON2, 0x00);
#if ENC28J60_FULL_DUPLEX
	// Pad to 60 bytes and append CRC.  Set half-duplex mode
	enc28j60_write_reg(spi, MACON3, MACON3_PADCFG0 | MACON3_TXCRCEN | MACON3_FRMLNEN | MACON3_FULDPX);
	// Back-to-back inter-packet gap.  0x12 for half-duplex, 0x15 for full-duplex
	enc28j60_write_reg(spi, MABBIPG, 0x15);
#else
	// Pad to 60 bytes and append CRC.  Set half-duplex mode
	enc28j60_write_reg(spi, MACON3, MACON3_PADCFG0 | MACON3_TXCRCEN | MACON3_FRMLNEN);
	// For half-duplex DEFER should be set for 802.3 compliance
	enc28j60_write_reg(spi, MACON4, MACON4_DEFER);
	// Back-to-back inter-packet gap.  0x12 for half-duplex, 0x15 for full-duplex
	enc28j60_write_reg(spi, MABBIPG, 0x12);
#endif
	// Non-back-to-back inter-packet gap
	enc28j60_write_reg16(spi, MAIPGL, 0x0c12);
	// Set maximum frame length
	enc28j60_write_reg16(spi, MAMXFLL, MAX_FRAMELEN);

	// Bank 3 (MAC address)
	enc28j60_write_reg(spi, MAADR1, enc28j60_macaddr[0]);
	enc28j60_write_reg(spi, MAADR2, enc28j60_macaddr[1]);
	enc28j60_write_reg(spi, MAADR3, enc28j60_macaddr[2]);
	enc28j60_write_reg(spi, MAADR4, enc28j60_macaddr[3]);
	enc28j60_write_reg(spi, MAADR5, enc28j60_macaddr[4]);
	enc28j60_write_reg(spi, MAADR6, enc28j60_macaddr[5]);

	// Set up LEDs - Bits 11-8 are the orange LED, bits 7-4 are the green one
	enc28j60_write_phy(spi, PHLCON, ENC28J60_LED_MODE);
	// Configure PHY
#if ENC28J60_FULL_DUPLEX
	enc28j60_write_phy(spi, PHCON1, PHCON1_PDPXMD);
	enc28j60_write_phy(spi, PHCON2, 0);
#else
	enc28j60_write_phy(spi, PHCON1, 0);
	enc28j60_write_phy(spi, PHCON2, PHCON2_HDLDIS);
#endif
...
//	enc28j60_set_bits(spi, EIE, EIE_INTIE /*| EIE_LINKIE */| EIE_PKTIE);
	...
	enc28j60_set_bits(spi, ECON1, ECON1_RXEN);

#if (ENC28J60_DUMP_REGS==1)
	enc28j60_dump_regs(spi);
#endif
...
int nic_recv(spi_t *spi, uint8_t *buf, unsigned int length)
{
	...
	if (enc28j60_read_reg(spi, EPKTCNT) <= 0)
	{
		//release SPI bus
		spi_release(spi);
	...
	status |= enc28j60_write_reg16(spi, ERDPTL, enc28j60_next_packet);

	// Read the header so we can determine the packet length
	status |= enc28j60_read_buf(spi, (uint8_t*)&rxstatus, sizeof(rxstatus));
	...
	if (rxstatus.status & ENC28J60_RXSTATUS_OK) {
		...
		status |= enc28j60_read_buf(spi, buf, length);
	} else {
		// Packet is bad
	...
	status |= enc28j60_write_reg16(spi, ERXRDPTL, enc28j60_rxrdpt_fix(enc28j60_next_packet));

	// Decrement EPKTCNT to acknowledge the packet
	status |= enc28j60_set_bits(spi, ECON2, ECON2_PKTDEC);
...
int nic_send(spi_t *spi, const uint8_t *buf, unsigned int length)
{
	...
	status |= enc28j60_write_reg16(spi, EWRPTL, TXSTART_INIT);
	...
	status |= enc28j60_write_reg16(spi, ETXNDL, txend);

	// Write the control byte (always 0)
	status |= enc28j60_write_buf(spi, (const uint8_t *)&dummy, 1);

	// Write the packet data
	status |= enc28j60_write_buf(spi, buf, length);

	// Start transmission
	status |= enc28j60_set_bits(spi, ECON1, ECON1_TXRST); /* errata 10 */
	status |= enc28j60_clear_bits(spi, ECON1, ECON1_TXRST);
	status |= enc28j60_clear_bits(spi, EIR, EIR_TXIF | EIR_TXERIF);
	status |= enc28j60_set_bits(spi, ECON1, ECON1_TXRTS);
	if (status < 0)
	{
		//release SPI bus
		spi_release(spi);
	...
	do
	{
		status = enc28j60_read_reg(spi, EIR);
		if (status < 0)
		{
			//release SPI bus
			spi_release(spi);
	...
	status = enc28j60_clear_bits(spi, ECON1, ECON1_TXRTS);

	// Read the TSV and determine actual length sent
	status |= enc28j60_write_reg16(spi, ERDPTL, txend + 1);
	status |= enc28j60_read_buf(spi, (uint8_t*)&txstatus, sizeof(txstatus));
...
int nic_keep_alive(spi_t *spi)
{
    ...
    int econ1 = enc28j60_read_reg(spi, ECON1);
